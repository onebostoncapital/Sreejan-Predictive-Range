...
if status:
    ...
    <div class="ai-card">
        ...
        <div class="news-ticker">
            ...
            {"".join([f'<div style="margin-top:5px;">• {n["title"]} <a style="color:{accent}; font-weight:bold;" href="{n["url"]}" target="_blank">[Read]</a></div>' for n in news_items]) if news_items else "Scanning..."}
        </div>
    </div>
    """, unsafe_allow_html=True)

    # 6. SECTION B: MANUAL ENGINE (Rule 17/30 - MASTER LOCK)
    st.markdown('<div class="manual-card">', unsafe_allow_html=True)
    st.subheader("✍️ Manual Range Control & Profitability Engine")
    
    if 'vault_range' not in st.session_state:
        st.session_state.vault_range = (float(auto_l), float(auto_h))

    m_range = st.slider("Adjust Manual Profit Zone", 
                        float(price*0.1), float(price*1.9), 
                        value=st.session_state.vault_range, 
                        step=0.01, key="manual_slider_persistent")
    
    st.session_state.vault_range = m_range
    m_l, m_h = m_range
    ...
    # 7. DUAL PROFIT CALCULATOR (Rule 17)
    ...
    # 8. RULE 31: THE CONFIGURATION LEDGER
    st.divider()
    config_summary = f"""--- SOLANA STRATEGY CONFIGURATION ---
Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
Capital: ${cap:,.2f}
Leverage: {lev}x
Liquidation Floor: ${liq_p:,.2f}
Manual Low: ${m_l:,.2f}
Manual High: ${m_h:,.2f}
Range Width: ${manual_w:,.2f}
Daily Expected Yield: ${res_m['1 Day']:,.2f}
--------------------------------------"""
    
    st.sidebar.download_button(
        label="📥 Download Strategy Ledger",
        data=config_summary,
        file_name=f"SOL_Strategy_{datetime.now().strftime('%m%d_%H%M')}.txt",
        mime="text/plain"
    )
    ...
    if m_l <= liq_p:
        ...
        st.error(f"🚨 ALERT: Manual Low crossed Liquidation Floor!")
    else:
