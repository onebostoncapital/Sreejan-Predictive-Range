...
# 1. STYLE ENGINE (LOCKED & RESTORED FROM SCREENSHOTS)
...
<style>
    ...
    /* AI Card Restored to Screenshot 1 Perfection */
    ...
    .tech-pulse {{ border: 1px dashed {accent}; padding: 12px; border-radius: 10px; background: rgba(212, 175, 55, 0.05); margin-bottom: 20px; }}
...
def get_stats(sol_df, b_df):
    # Initialize with "Loading..." if data is missing
    stats = {"sol": ["---"]*5, "btc": ["---"]*3}
...
if status:
    ...
    # --- PERFORMANCE RIBBON (LOCKED LAYOUT) ---
    ...
    liq_p = price * (1 - (1/lev)*0.45) # Restored logic

    # --- AI CARD (RE-INTEGRATED LIQUIDATION BOX) ---
    st.markdown(f"""
    ...
    # --- NEWS & MANUAL (RESTORED POSITIONS) ---
