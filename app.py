...
import feedparser  # Ensure you have run: pip install feedparser
...
def fetch_live_market_data():
    try:
        ...
        if 'bitcoin' in p_data and 'prices' in h_data:
            btc_p, sol_p = p_data['bitcoin']['usd'], p_data['solana']['usd']
            prices = [p[1] for p in h_data['prices']]
            current_atr = np.std(prices) * 1.5 
            stats = {"high": max(prices), "low": min(prices), "avg": np.mean(prices), "t_high": sol_p * 1.01, "t_low": sol_p * 0.99}
...
# --- NEW MODULE: LIVE TECHNICAL SENTINEL (LOCKED) ---
...
def get_yield_df(l, h, lev):
    ...
    return pd.DataFrame({"Timeframe": ["1 Hour", "3 Hours", "1 Day", "1 Week", "1 Month"], "Projected Profit": [f"${base:,.2f}", f"${base*3:,.2f}", f"${base*24:,.2f}", f"${base*168:,.2f}", f"${base*720:,.2f}"]})
...
# --- MODULE 7: PERSISTENT NEWS (REBUILT FOR STABILITY) ---
st.markdown("---")
st.markdown("### 📰 Solana Ecosystem Live News")
try:
    ...
    if feed.entries:
        ...
        for i, entry in enumerate(feed.entries[:4]):
            target = n_col1 if i % 2 == 0 else n_col2
            with target:
                st.info(f"🔗 **{entry.title}**\n\n[Read full article]({entry.link})")
except:
    st.error("News Feed is refreshing. Please wait...")
...
st.toast("Strategy Auto-Saved", icon="💾")
